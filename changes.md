...
## 6. Elite Scope Expansion (Corporate Intelligence)
- **EVENT Module:** Implemented a functional corporate action crawler in `backend/event_scraper.py` that ingests live Mergers, Acquisitions, and E-IPO data from IDX.
- **Fundamental Enrichment:** Enriched the terminal with institutional metrics beyond insiders, including Market Cap, PE Ratio, and PB Ratio.
- **Visual Intelligence:** Enhanced the Sector Heatmap with Visual Flow Bars and "Sector Weight" (Market Cap) context for intuitive information delivery.
- **AI Pivot:** Migrated the intelligence layer to the high-performance `nemotron-mini-4b-instruct` model, triggered on-demand via institutional stock clicks.
